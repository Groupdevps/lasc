										...
										>
											...
											>							
												...
												<v-text-field
													...
													v-if 	= "['text'].includes(item_field.type)"
													:class 	= "item_field.class"
												></v-text-field>
												<v-text-field
													dense
...
export default {
	...
	methods : {
			...
			if (action.action == "confirm") {
				...
				if (this.requests) {
					if (this.requests.action == "edit" ) {
						this.update(this.info.content);
					}else 
					if (this.requests.action == "create") {						
						this.save(this.info.content);
					}else
		...
		save(item){
			if (this.info_related_key && this.info_related_key.main_id) {											
				this.loadings.content = true;
				...
				this.$Axios.post(api, item).then(res => {
					...
					if (res && res.data) {
						this.$EventBus.$emit("notifications",{
							type 	: "saved",
							message : this.$t("information"),
						});
						// if (this.requests && this.requests.close_with_req) {
							this.actions("close_modal", res.data);
				...
				this.actions( {action :"close_modal"}, {...item})
