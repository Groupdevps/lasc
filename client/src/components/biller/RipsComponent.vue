...
import SubRipComponent from "@/components/biller/SubRipComponent.vue"

export default {
	name : "RipsComponent",
	props :["currentHeight"],
	components 	: { SubRipComponent },
	data 	: (vm) => ({
		loading : false,
		infoItem:null,
		info : { uvt : null},
		...
		headers 		: [
			{ text : "FECHA INICIAL" , value: "startDate"}, // input | output
			{ text : "FECHA FINAL", value  : "endDate" },
			{ text : "EPS" , value: "currentAdministratorName"},					
		...
		methods:{
			...
			async getList(){
				try{
					...
					console.warn("Error get rip ", error)
				}finally{
					this.loading = false;
				}
			},
			async deleteItem(){
				try{
					...
					if (this.$Helper.isPermission(this.currentPermission, "canDelete")) {
						...
						if (res && res.error) {
							this.$EventBus.$emit("notifications", { type : "warning" , msg : this.$Helper.renderErrorMessage(res.error, "Error Eliminando RIP")});
						}else
						if (res && res.data) {
					...
					this.$EventBus.$emit("notifications",{
						type : "error",
						msg :  this.$Helper.renderErrorMessage(error, "Eliminando RIP")
					})
				}finally{
					this.loading = false;
				}
			...
			async disabledItem(){
				try{
					...
					if (this.$Helper.isPermission(this.currentPermission, "canEdit")) {
						...
						if (res && res.error) {
							this.$EventBus.$emit("notifications", { type : "warning" , msg : this.$Helper.renderErrorMessage(res.error, "Error Deshabilitando RIP")});
						}else
						if (res && res.data) {
							const idx = this.list.findIndex(x => x.id == this.infoItem.id)
							if (idx >= 0) {
					...
					this.$EventBus.$emit("notifications",{
						type : "error",
						msg :  this.$Helper.renderErrorMessage(error, "Deshabilitando RIP")
					})
				}finally{
					this.loading = false;
				}
