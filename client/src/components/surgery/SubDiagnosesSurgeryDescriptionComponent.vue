	...
	>
		<template v-slot:top>
			<v-row dense v-show="!isDisabled">
				<v-col cols="12">
					<CategoryComponent startCategory="DIAGNOSTICOS" :inputField="searchInput" keyField="cup" :isDisabledCategory="true"></CategoryComponent>
				</v-col>
...
<script type="text/javascript">
	...
	import QxDiagnosticList from "@/services/QxDiagnosticListService.js"
	import dates 			from "@/helpers/dates.vue"

	export default{
		name : "SubDiagnosesSurgeryDescriptionComponent.",
		...
		data : (vm)=>({	
			...
			info : {
				...
				DiagnosticListId : undefined,
				DiagnosticList : null,
			},			
			searchInput : { cup : {}, category : {}}, 
			reference : "SubMedicalNotes",						
			headers : [
				{ text : "CODIGO", value : "DiagnosticList.code" },
				{ text : "DESCRIPCION", value : "DiagnosticList.description" },//hour			
		...
		methods:{
				...
				try{
					...
					if (this.$Helper.isPermission(this.currentPermission, "canAdd")) {
						if (this.info && !this.info.description ) {
							this.$EventBus.$emit("notifications", { type : "warning" , msg : "Se requiere Diagnostico"});
						}else{													
							this.info.UserId=this.$store.getters["auth/userId"];
							if (this.inputField && this.inputField.id) {
								const service = new QxDiagnosticList();
								const res = await service.saveInfo(this.info)								
								if (res) {
									if (res.error){
										this.$EventBus.$emit("notifications", { type : "warning" , msg : this.Helper.renderErrorMessage(res.error, "Error creando Diagnostico")});
									}else if (res.data) {									
					...
					this.$EventBus.$emit("notifications", {
						type: "warning",
						msg : "Error agregando Diagnostico"
					});
				}finally{
					this.loading = false;
				}
			...
			async updateItem(){
				try{
						...
						if (this.info && !this.info.description ) {
							...
							if (this.inputField &&  this.inputField[this.keyField] &&  this.inputField[this.keyField].length) {		
								if (this.info && this.info.id ) {									
									...
									if (res) {
										if (res.error){
											this.$EventBus.$emit("notifications", { type : "warning" , msg : this.$Helper.renderErrorMessage(res.error, "Error Editando Diagnostico")});
										}else if (res.data) {	
					...
					this.$EventBus.$emit("notifications", {
						type: "warning",
						msg : "Error Editando Diagnostico"
					});
				}finally{
					this.loading = false;
				}
			...
			async deleteItem(){
				try{
					...
					this.$EventBus.$emit("notifications", {
						type: "warning",
						msg : "Error eliminando Diagnostico"
					});
				}finally{
					this.loading = false;
				}
