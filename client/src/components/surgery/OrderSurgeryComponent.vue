<template>
	<FormSessionLayout
		:fields="fields"
		title=""
		subtitle="Ordenes de Cirugia"
		reference = "OrdenSurgeryForm"
		:inputField="info"
	...
	>
		...
		<template v-slot:field-Orders="{ itemField }">
			<SubSurgeryComponent :infoInput="info" :currentPermission="currentPermission"></SubSurgeryComponent>
		</template>
	</FormSessionLayout>	
</template>
<script type = "text/javascript">
import field_params from "@/components/medico/js/surgery_params.js"
import SubSurgeryComponent from "@/components/medico/SubSurgeryComponent.vue"
...
export default {
	name : "OrdenSurgeryComponent",
	props: ["ReduceSize"],
	components : {
		FormSessionLayout, SubSurgeryComponent
	}, //components
	data : (vm) => ({		
		info 		 	: {
	...
	computed : {
		...
		currentPermission(){
			return this.$store.getters["auth/getPermission"]("OrderSurgery") 
		}
	}, // computed
	...
	methods : {
		...
		async getInfo(){
			try {
				this.loading = true;
				const typeOrder = this.typesOrder.filter(x=> x.name == "ORDEN CIRUGIA");				
