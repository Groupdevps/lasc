<template>
	<FormSessionLayout
		:fields="fields"
		title=""
		subtitle="Notas de Evolucion"
		reference = "EvolutionNotesSurgeryForm"
		:inputField="info"
...
export default {
	name : "EvolutionSurgeryComponent",
	props: ["ReduceSize"],
	components : {
	...
	computed : {
		...
		currentPermission(){
			return this.$store.getters["auth/getPermission"]("MedicalNotes") //EvolutionSurgery 
		}
	}, // computed
		...
		async getInfo(){
			try {
				...
				if (res && res.data && res.data.length) {
					...
					this.info.ObservationEvolutionNotes = res.data && res.data.length ? res.data : []; // .ObservationEvolutionNotes	
					this.getDiagnoses();
				}else{
					this.getDiagnoses();
