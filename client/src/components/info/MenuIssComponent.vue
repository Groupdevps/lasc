    ...
    export default{
        ...
        watch: {
            search (val) {
                ...
                this.$store.dispatch("listISS/findList", {
                    search : val,
                    limit : 30                                        
                }).then(res=>{
                    this.loading = false;
        ...
        computed:{
            ...
            renderFilterList(){
                return this.$store.getters["listISS/list"];
            },
            renderMessageHint(){
               let text="";
        ...
        methods:{
            onMenu(isOpen) {                            
                if (isOpen) {
                    // El menÃº se ha abierto, esperamos un tick para que se renderice el DOM
                    this.$nextTick(() => {
                        const clearTime = setTimeout(() => {
                            this.adjustMenuWidth();
                            // this.addScrollListener(); // Agregar listener de scroll al abrir el menÃº
            ...
            onCloseMenu(){                
                // El menÃº se ha cerrado, removemos el event listener
                const menuElement = document.querySelector('.v-menu__content');
                if (menuElement) {
                    menuElement.removeEventListener('scroll', this.onScroll);                    
                    // this.removeScrollListener(); // Remover listener de scroll cuando el menÃº se cierra
                }
            },
            onScroll(event) {
                ...
                if (scrollElement.scrollTop + scrollElement.clientHeight >= scrollElement.scrollHeight - 5) {                    
                    ...
                    if (!this.loading) {
                        ...
                        if (this.search) {
                            ...
                            this.$store.dispatch("listISS/getList").then(res=>{
            ...
            adjustMenuWidth(){
                this.$nextTick(() => {
                    const autocompleteWidth = this.$refs.autocompleteMenuISS.$el.clientWidth; // Obtener el ancho del autocomplete
                    const menuElement = document.querySelector('.v-menu__content');
                    this.maxWidthMenu=autocompleteWidth
                    if (menuElement) {
                        menuElement.style.minWidth = `${autocompleteWidth}px`; // Establecer el ancho del menÃº
                        menuElement.style.maxWidth = `${autocompleteWidth}px`;
                        menuElement.style.left = `${this.$refs.autocompleteMenuISS.$el.getBoundingClientRect().left}px`;
            ...
            handleWindowScroll(event) { 
                // Verificar si el menÃº estÃ¡ abierto y cerrar si hay scroll fuera
                const autocomplete = this.$refs.autocompleteMenuISS.$el;
                const menuElement = document.querySelector('.v-menu__content');                

                if (!autocomplete.contains(event.target) && !menuElement.contains(event.target)) {
                    this.$refs.autocompleteMenuISS.blur(); // Forzamos el blur para cerrar el menÃº
                }
                
            },
            getList(){
                if (!this.$store.getters["listISS/isList"]) {
                    this.$store.dispatch("listISS/getList").then(res=>{
                        this.loading = false;
                    });
                }else{
