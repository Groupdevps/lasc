...
<script>    
    import TypeRelationshipService from "@/services/TypeRelationshipService.js"
    export default{
        name : "MenuTypeRelationshipComponent",
        ...
        methods:{
            ...
            async getList(){
                try {
                    this.loading = true;
                    const service = new TypeRelationshipService();
                    const res = await service.getInfo();
                    if (res && res.data && res.data.length > 0) {
                        this.list = res.data;
                    }
                } catch (error) {
                    console.warn("error get info type relationship ", error)
                }finally{
                    this.loading = false;
                }
            },
            async getSearch(params){
                ...
                //     }
                // } catch (error) {
                //     console.warn("error search info type relationship ", error)
                // }finally{
                //     this.loading = false;
                // }
            },
