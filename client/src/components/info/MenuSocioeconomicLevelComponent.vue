<template>
    <v-select
        ...
        item-value = "id"
        item-text = "name"
        prepend-inner-icon="mdi-relative-scale"    
...
<script>    
    export default{
        name : "MenuSocioeconomicLevelComponent",
        ...
        computed:{
            ...
            renderList(){
                return this.$store.getters["listSocioeconomicLevel/list"];
            },
        ...
        methods:{
            ...
            async getList(){
                try {
                   if (!this.$store.getters["listSocioeconomicLevel/isList"]) {
                        this.$store.dispatch("listSocioeconomicLevel/getList").then(res=>{
                            this.loading = false;
                        });
                    }else{
                        this.loading= false
                    ...
                    console.warn("error get info socioeconomic level ", error)
                }finally{
                    this.loading = false;
                }
            },
            async getSearch(params){
                 try {
                    ...
                    console.warn("error search info socioeconomic level ", error)
                }finally{
                    this.loading = false;
                }
            },
            async save(){
                try{
                    ...
                    console.warn("error save info socioeconomic level ", error)
                }finally{
                    this.loading = false;
                }
            }
