<template>
	...
	>
		...
		<template v-slot:item.CAU = "{ item, index}">
			<v-btn small dark :color = "item.colorCAU" class ="pa-1 pb-3"> 
				<v-input
					error-count ="2"
					disabled					
					error			
		...
		<template v-slot:item.actions="{item}" >			
			<v-btn  small color="primary" v-show ="isShowToRoles" @click="redirect({...item, redirectTo: 'DashboardMedico'})"> <v-icon small>mdi-stethoscope</v-icon></v-btn>
		</template>
	</v-data-table>
...
<script type="text/javascript">
	...
	export default{
		...
		computed:{
			...
			currentPermission(){
				return this.$store.getters["auth/getPermission"]("AgendaMedicaComponent") // this.$route.name
			},
			isShowToRoles(){
