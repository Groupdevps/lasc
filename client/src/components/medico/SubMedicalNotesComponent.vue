						...
						if (this.info && !this.info.note ) {
							...
							if (res) {
								...
								if (res.error){
									...
									this.infoInput.ObservationEvolutionNotes.push({...this.info, id: res.data.id});
									this.$EventBus.$emit("notifications", {
										type: "success",
										msg : "Nota Agregada"
									});
				...
				try{
					...
					if (this.$Helper.isPermission(this.currentPermission, "canEdit")) {
						if (this.info && !this.info.note ) {
							...
							if (this.infoInput &&  this.infoInput.ObservationEvolutionNotes &&  this.infoInput.ObservationEvolutionNotes.length) {		
							...
							if (this.infoItem && this.infoItem.id ) {
								...
								if (res) {
									if (res.error){
										...
										const idx = this.infoInput.ObservationEvolutionNotes.findIndex(x => x && (x.id == this.infoItem.id )); // || (this.infoItem.idx && x.idx == this.infoItem.idx)
										if (idx >= 0) {
