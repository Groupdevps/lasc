<template>
	<FormSessionLayout
		:fields="fields"
		title=""
		subtitle="Procedimientos No Quirurgicos"
		reference = "NonSurgicalProceduresForm"
		:inputField="info"
	...
	>
		...
		<template v-slot:field-Orders="{ itemField }">
			<SubNonSurgicalProcedures :infoInput="info" :currentPermission="currentPermission"></SubNonSurgicalProcedures>
		</template>
	</FormSessionLayout>	
</template>
<script type = "text/javascript">

import field_params from "@/components/medico/js/non_surgical_procedures_params.js"

import DiagnosesService from "@/services/DiagnosesService.js"
import OrderService 		from "@/services/OrderService.js"

import SubNonSurgicalProcedures from "@/components/medico/SubNonSurgicalProceduresComponent.vue"
...
export default {
	name : "NonSurgicalProceduresComponent",
	props: ["ReduceSize"],
	components : {
		FormSessionLayout, SubNonSurgicalProcedures
	}, //components
	data : (vm) => ({		
		info 		 	: {
		...
		fields 		: field_params,		
		diagnoses  	: [],		
		reference 	: 'NonSurgicalProcedures',		
		loading : false,
	...
	computed : {
		...
		currentPermission(){
			return this.$store.getters["auth/getPermission"]("NonSurgicalProcedures") 
		}
	}, // computed
	...
	methods : {
		...
		async getInfo(){
			try {
				this.loading = true;
				const typeOrder = this.typesOrder.filter(x=> x.name == "ORDEN PROCEDIMIENTOS");				
