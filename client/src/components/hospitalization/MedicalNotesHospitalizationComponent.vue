<template>
	<FormSessionLayout
		:fields="fields"
		title=""
		subtitle="Notas Medicas Hospitalizacion"
		reference = "MedicalNotesForm"
		:inputField="info"
	...
	>
		...
		<template v-slot:field-ObservationEvolutionNotes="{ itemField }">
			<SubMedicalNotesHospitalizationComponent :infoInput="info"></SubMedicalNotesHospitalizationComponent>
		</template>
	</FormSessionLayout>	
</template>
<script type = "text/javascript">
import field_params from "@/components/hospitalization/js/medical_notes_hospitalization_params.js"
import SubMedicalNotesHospitalizationComponent from "@/components/hospitalization/SubMedicalNotesHospitalizationComponent.vue"
import FormSessionLayout from "@/layouts/FormSessionLayout.vue"
import DiagnosesService from "@/services/DiagnosesService.js"
import MedicalNotesHospitalizationService from "@/services/MedicalNotesHospitalizationService.js"
export default {
	name : "MedicalNotesHospitalizationComponent",
	components : {
		FormSessionLayout, SubMedicalNotesHospitalizationComponent
	}, //components
	props: ["ReduceSize"],
	data : (vm) => ({		
		info 		 	: {
	...
	methods : {
		...
		async getInfo(){
			try {
				this.loading = true;	
				const service = new MedicalNotesHospitalizationService();
				...
				if (res && res.data) {
					...
					this.info.ObservationEvolutionNotes = res.data.ObservationEvolutionNotes;	
					this.getDiagnoses();
				}else{
					this.getDiagnoses();
