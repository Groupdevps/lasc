...
export default class RecommendationsService {
    ...
    async findInfo(params){
        try {
            const response = await axios.post(`/api/v1/find/recommendation/`, params) 
            return response       
        } catch (error) {
            return {error}            
        }
    ...
    async saveInfo(params){
        try {
            const response = await axios.post(`/api/v1/recommendation`, params) 
            return response       
        } catch (error) {
            return {error}            
        }
    ...
    async updateInfo(params){
        try {
            const response = await axios.put(`/api/v1/recommendation/${params.id}`, params) 
            return response       
        } catch (error) {
            return {error}            
        }
    ...
    async downloadPDF(params){
        try {            
            ...
            const response = await axios.requestAxios({
                url: `/api/v1/recommendation/${params.id}/pdf`,
                method: 'GET',
            ...
            if(response && response.data){ 
                Helper.downloadFilePdf(response.data, `${params.id}-recommendation.pdf`);
                return { data : "OK" };
            }
        } catch (error) {
    ...
    linkPdf(params){
        return `/api/v1/recommendation/${params.id}/pdf`
    }
