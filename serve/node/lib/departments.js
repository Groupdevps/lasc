const { CombedTextLayoutError } = require("pdf-lib");

const list = [
	{"region":"Región Eje Cafetero - Antioquia","c_digo_dane_del_departamento":"5","departamento":"Antioquia","c_digo_dane_del_municipio":"5.001","municipio":"Medellín"}
...
let temp = []
const departments = list.filter(x => {
	if (!temp.includes(x.departamento)) {
		temp.push(x.departamento);
		return x 
	}
}).map(z => { 
	return { 
		
		name 		: z.departamento.toUpperCase(),
		code 		: z.c_digo_dane_del_departamento,
		createdAt   : new Date(),
      	updatedAt   : new Date()
	}
});
const cities = list.map(x => {  return {
	name 		: x.municipio.toUpperCase(),
	code		: x.c_digo_dane_del_municipio,
	StateId 	: temp.findIndex(z => (z == x.departamento)) + 1,
	createdAt   : new Date(),
  	updatedAt   : new Date()
}});
