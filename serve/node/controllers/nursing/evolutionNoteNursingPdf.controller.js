    ...
    result.core = {
        ...
        subject: [
            ...
            ...(result.ExpenseSheet ? [{
                ...
                header: ['departureDate', 'hospitalization', 'accountCut', 'Attention?.HistoryPerson?.fullName', 'Attention?.HistoryPerson?.numberId', 'Attention?.assignedDate', 'Attention?.egressDate'],
                title: ['Salida General', 'Hospitalizacion', 'Corte de cuenta', 'Paciente', 'Documento', 'Fecha ingreso', 'Fecha egreso'],
                data: data.ExpenseSheet,
                isHorizontal: true
            }] : []), 
...
const getExpenseSheetToPdf = async (req, res) => {
    try {
        ...
        const supplies = await Apply.findAll({
            where: { AttentionId: attention },
            include: [{ model: SubSupply, required: true, include: [{ model: Concept, include: [{ model: Product }] }] }]
        });
...
function groupSuppliesByProduct(inputJSON) {
    ...
    inputJSON.forEach(supply => {
        supply.SubSupplies.forEach(subSupply => {
            const product = subSupply.Concept.Product;
...
function groupProceduresByCupsList(inputJSON) {
    ...
    inputJSON.forEach(supply => {
        supply.SubSupplies.forEach(subSupply => {
            const cupsList = subSupply.Concept.CupsList; // Obtener CupsList
